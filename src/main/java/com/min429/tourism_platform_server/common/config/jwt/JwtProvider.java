...
public class JwtProvider {
	...
	public void ValidateToken(String token) { // 검증하려면 token에서 "Bearer " 없어야 됨
		try {
			...
			throw new TokenValidationException(ErrorCode.TOKEN_EXPIRED);
		} catch (Exception e) {
			throw new TokenValidationException(ErrorCode.INVALID_TOKEN);
		}
	}
	...
	public String getToken(String authorizationHeader) {
		if (authorizationHeader == null)
			throw new InternalException(ErrorCode.INTERNAL_SERVER);
