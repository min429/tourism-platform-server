...
$(document).ready(function () {
    ...
    let storedImageUrls = []; // 기존에 저장되어있던 이미지들
    let tags = [];  // 태그를 저장할 배열
    ...
    let boardId = urlParams.get('boardId');  // 'boardId' 값 추출
    let type = '';
    ...
    $(document).ready(function () {
        ...
        // 게시글 데이터를 불러오기
        fetch(`/api/user/boards/content/${type}?boardId=${boardId}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                // 카테고리 선택 렌더링 (기존 카테고리 선택 상태 반영)
                if (type === 'spot') {
                    $("#spot-button").addClass('active');
                } else if (type === 'restaurant') {
                    $("#restaurant-button").addClass('active');
                } else if (type === 'accommodation') {
                    $("#accommodation-button").addClass('active');
                }

                // 이미지 렌더링
                data.images.forEach(imageUrl => {
                    let imgElement = $(`
                    <div id="stored-images" class="review-image-thumbnail position-relative me-2 bg-color">
                        <img class="rounded" src="${imageUrl}" alt="Image" style="width: 100px; height: 100px;">
                ...
                `);
                    ...
                    imgElement.find("button").on("click", function () {
                        imgElement.remove();
                        storedImageUrls = storedImageUrls.filter(img => img !== imageUrl);  // 배열에서 제거
                    });
                ...
                data.tags.forEach(tag => {
                    addTagToUI(tag);  // 태그 UI 추가
                    tags.push(tag);  // 태그 배열에 추가
                });

                // 지도 렌더링
                registerMap(data.addr1);
            })
    ...
    function addImagePreview(file) {
        ...
        reader.onload = function (e) {
            let imgElement = $(`
                <div id="new-images" class="review-image-thumbnail position-relative me-2 bg-color">
    ...
    $("#submit-post").on("click", function () {
        ...
        // 기존에 저장되어있던 이미지들 추가
        storedImageUrls.forEach(storedImage => {
           formData.append("imageUrls", storedImage);
        });
        ...
        // 폼 데이터를 서버로 전송
        $.ajax({
            url: `${baseUrl}/user/boards/update/${type}?area=${area}&boardId=${boardId}`,
            type: 'POST',  // 'PUT' 대신 'POST'로 변경
            data: formData,
            contentType: false,
            processData: false,
            success: function () {
            ...
            error: function (error) {
                // 전송 실패 시 처리
                console.error('Error creating post', error);  // 에러 메시지 수정
            }
        });
    });
});
